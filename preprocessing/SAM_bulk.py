...
# FOR MNRF
    one = df[df['Tarfile']=="NE120-122"]
    two = df[df['Tarfile']=="NW1211-1232"]
    three = df[df['Tarfile']=='NW1301-1328']
    four = df[df['Tarfile']=='S']
    five = df[df['Tarfile']=='Unk1']
    MNRFdf = [one, two, three, four, five]
    df = pd.concat(MNRFdf, ignore_index=True)
    ...
    for i in df.index:
        ...
        path = df['path'][i]
#ahc        path = path.split('/')[3]
        root, year, area, filename = path.split('/') #mnrf
        path = area + '~' + filename #mnrf
        path = path[:-4]
        spp = df['species'][i]
